...
從 ONTAP 9 。 13.1 開始、您可以在執行磁碟區複製建立作業時、建立 FlexClone 、並將 SnapLock 類型選項設為「非 SnapLock 」、並將 Snapshot 複本指定為「父快照」、以即時還原 SnapLock 資料保險箱關係目的地 SnapLock 磁碟區上的鎖定 Snapshot 複本。深入瞭解 link:../../ontap/volumes/create-flexclone-task.html?q=volume+clone["使用 SnapLock 類型建立 FlexClone Volume"^]。
