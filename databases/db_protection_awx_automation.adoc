...
'''
側欄：側欄Permalink：Databases/db_protection_awx_automation.html關鍵字：Linux、RHEL Oracle19c、NFS、ONTAP 效益摘要：本頁說明NetApp ONTAP ®資料儲存設備上Oracle19c的自動化資料保護。-
...
.記錄磁碟區的複寫
--
stdin>中的未解析指令-包括：..//_iinclude/db_protection _log_replication.adication[]
