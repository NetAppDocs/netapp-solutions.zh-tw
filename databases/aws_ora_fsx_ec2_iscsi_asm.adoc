...
[%collapsible]
====
在已配置先決條件的情況下、以 root 使用者身分登入 EC2 執行個體、以設定 Linux 核心以進行 Oracle 安裝。
...
[%collapsible]
====
從 FSX 主控台配置三個磁碟區、以裝載 Oracle 資料庫二進位、資料和記錄檔。
...
. 以 EC2 使用者身分透過 SSH 登入 EC2 執行個體。然後變更為 SSH 金鑰和 EC2 執行個體 IP 位址。
+
....
...
....
. 以 Oracle 使用者身分啟動 `gridSetup.sh` 適用於網格基礎架構安裝。
+
....
...
....
. 如果您收到訊息「 [INS-43080] Sas the configuration Assistants failed 、 was canceled or bipped 」（ [INS-43080] 部分組態助理失敗、已取消或略過）、請以 Oracle 使用者身分執行下列命令、檢查是否已建立資料磁碟群組資源。
+
....
bin/crsctl stat res -t
....
. 如果未建立資料磁碟群組、請以 root 使用者身分重新載入多路徑。
+
....
systemctl restart multipathd
....
. 身為 Oracle 使用者、如果資料磁碟群組並非在網格安裝期間建立、請建立該群組。
+
....
...
....
. 執行 `root.sh` 僅軟體安裝後的指令碼。
+
....
/u01/app/oracle/product/19.0.0/db1/root.sh
....
. 建立 `dbca.rsp` 包含下列項目的檔案：
+
....
