...
在本文件中、我們將示範如何運用應用程式層級的PostgreSQL串流HA/DR解決方案、並針對ONTAP 資料庫檔案和EC2運算執行個體（例如PostgreSQL DB伺服器）建置以AWS FSX更新儲存設備為基礎的PostgreSQL HA/DR解決方案。相較於HA/DR的傳統PostgreSQL應用程式層級串流複寫、此解決方案可建立更簡單且可媲美的系統、並提供同等的結果。
...
NetApp建議使用已建立完善的作業系統叢集軟體來執行手動容錯移轉、因為這可能需要授權。
