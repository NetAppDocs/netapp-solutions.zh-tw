...
有了EC2執行個體上的SQL Server、資料庫管理員就能存取及自訂其資料庫環境和基礎作業系統。EC2執行個體上的SQL Server搭配使用 https://["AWS FSX ONTAP 功能"^] 若要儲存資料庫檔案、請使用區塊層級複寫來實現高效能、資料管理、以及簡單易用的移轉路徑。因此、您可以在AWS VPC上執行複雜的資料庫、只需簡單的移轉方法、減少點擊次數、而且不需進行架構轉換。
...
此外、請設定 https://["空間分配"^] LUN上的選項設為啟用、以便在ONTAP 磁碟區空間不足、且磁碟區中的LUN無法接受寫入時、FSX功能會通知EC2主機。此外、當ONTAP EC2主機上的SQL Server刪除資料時、此選項可讓FSX for Sf2自動回收空間。空間配置選項預設為停用。
...
. 依照中所列的步驟建立Volume https://["在FSX上建立適用於NetApp ONTAP 的Volume"^]。
+
|===
...
. 若要使I/O與作業系統分割配置一致、請使用windows_2008做為建議的LUN類型。請參閱 https://["請按這裡"^] 以取得更多資訊。
...
如需版本相容性、請參閱 https://["NetApp 互通性對照表工具"^]。
...
[%collapsible]
====
請依照 https://["安裝SnapCenter 此伺服器"^] 和 https://["安裝SnapCenter 適用於Microsoft SQL Server的功能性外掛程式"^] 以安裝及設定SnapCenter 。
...
若要使用SnapCenter PowerShell自動建立iSCSI LUN及NetApp更新安裝、請從複製repo https://["此GitHub連結"^]。
