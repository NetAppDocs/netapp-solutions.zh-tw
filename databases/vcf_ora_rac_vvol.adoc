...
. 在 VCF 工作負載網域中、為三個 Linux VM 、兩個 VM 用於 Oracle RAC 資料庫叢集、以及一個 VM 用於 Ansible 控制器。配置一部 Windows 伺服器 VM 以執行 NetApp SnapCenter 伺服器。有關爲自動 Oracle 數據庫部署設置 Ansible 控制器的信息，請參閱以下資源 link:https://docs.netapp.com/us-en/netapp-solutions/automation/getting-started.html["NetApp解決方案自動化入門"^]。
...
* 適用於 VMware vSphere 的 ONTAP 工具文件
+
