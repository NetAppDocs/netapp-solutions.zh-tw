...
請記住、每個地區和服務類型只允許一個Active Directory連線。
...
| LM相容層級 | Active Directory連線所支援的驗證語言層級。請參閱「」一節 authentication dialects」以取得更多資訊。 | vLMvb-krb | 否 


| 傳入CIFS流量需要SMB加密 | 所有共用都需要SMB加密。這不是Cloud Volumes Service 由靜止使用；而是根據每個磁碟區設定加密（請參閱「」一節） share security features」）。 | 錯 | 否 
