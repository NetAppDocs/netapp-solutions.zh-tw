...
image::aws-fsx-guest-1.png[AWS FSX 客體 1]

. 在「Networking（網路）」區段中、針對Virtual Private Cloud（VPC）選擇適當的VPC和偏好的子網路、以及路由表。在此情況下、會從下拉式清單中選取vmcfsx2.VPC。


image::aws-fsx-guest-2.png[AWS FSX 客體 2]

. 對於建立方法、請選擇「標準建立」。您也可以選擇「快速建立」、但本文件使用「標準建立」選項。


image::aws-fsx-guest-3.png[AWS FSX 訪客 3.]

. 在「Networking（網路）」區段中、針對Virtual Private Cloud（VPC）選擇適當的VPC和偏好的子網路、以及路由表。在此情況下、會從下拉式清單中選取vmcfsx2.VPC。


image::aws-fsx-guest-4.png[AWS FSX 訪客 4.]
...
image::aws-fsx-guest-5.png[AWS FSX 訪客 5.]
...
image::aws-fsx-guest-6.png[AWS FSX 訪客 6.]

. 在「預設Volume組態」區段中、指定Volume名稱和大小。這是NFS Volume。若要提升儲存效率、請選擇「啟用」以開啟ONTAP 「不支援的儲存效率」功能（壓縮、重複資料刪除和壓縮）、或選擇「停用」以關閉這些功能。


image::aws-fsx-guest-7.png[AWS FSX 客體 7]

. 檢閱「Create File System（建立檔案系統）」頁面上顯示的檔案系統組態。
. 按一下建立檔案系統。


image::aws-fsx-guest-8.png[AWS FSX 訪客 8.]

image::aws-fsx-guest-9.png[AWS FSX 訪客 9.]

image::aws-fsx-guest-10.png[AWS FSX 客體 10]
...
image::aws-fsx-guest-11.png[AWS FSX 客體 11]
...
image::aws-fsx-guest-20.png[AWS FSX 客體 20]

. 執行後、請執行df命令來驗證掛載。


image::aws-fsx-guest-21.png[AWS FSX 客體 21]
...
image::aws-fsx-guest-22.png[AWS FSX 訪客 22.]

. 在「共享資料夾」工具中、選擇左窗格中的「共享」、即可查看Amazon FSX檔案系統的作用中共用。


image::aws-fsx-guest-23.png[AWS FSX 訪客 23.]

. 現在請選擇新的共用區、然後完成「建立共用資料夾」精靈。


image::aws-fsx-guest-24.png[AWS FSX 客體 24]

image::aws-fsx-guest-25.png[AWS FSX 客體 25]
...
image::aws-fsx-guest-26.png[AWS FSX 客體 26]
...
image::aws-fsx-guest-30.png[AWS FSX 客體 30]
...
image::aws-fsx-guest-31.png[AWS FSX 訪客 31]
...
image::aws-fsx-guest-32.png[AWS FSX 客體 32]
...
image::aws-cvo-guest-20.png[AWS CVO 訪客 20]
...
image::aws-cvo-guest-30.png[AWS CVO 訪客（ 30 ）]

image::aws-cvo-guest-31.png[AWS CVO 訪客 31]
...
image::aws-cvo-guest-32.png[AWS CVO 來賓 32]
...
image::aws-cvo-guest-33.png[AWS CVO 訪客 33]
...
image::aws-cvo-guest-34.png[AWS CVO 訪客 34.]
...
image::aws-cvo-guest-40.png[AWS CVO 訪客 40]

image::aws-cvo-guest-41.png[AWS CVO 訪客 41.]
