...
image::dr-vmc-aws-image1.png[DR vmc AWS 影像 1]
...
+
image::dr-vmc-aws-image10.png[DR vmc AWS 影像 10.]
...
....
+
image::dr-vmc-aws-image11.png[DR vmc AWS 影像 11.]
...
+
image::dr-vmc-aws-image12.png[DR vmc AWS 影像 12.]
...
.. 按一下「初始化叢集對等」以完成程序。
+
image::dr-vmc-aws-image13.png[DR vmc AWS 影像 13.]
...
....
+
image::dr-vmc-aws-image14.png[DR vmc AWS 影像 14.]
...
+
image::dr-vmc-aws-image15.png[DR vmc AWS 影像 15.]
...
** 目的地儲存VM
+
image::dr-vmc-aws-image16.png[DR vmc AWS 影像 16.]
...
image::dr-vmc-aws-image17.png[DR vmc AWS 影像 17.]
...
+
image::dr-vmc-aws-image18.png[DR vmc AWS 影像 18.]

. 在「Add Storage System（新增儲存系統）」對話方塊中、新增本機內部部署ONTAP 的元件叢集的管理IP位址、以及使用者名稱和密碼。然後按一下「提交」開始探索儲存系統。
+
image::dr-vmc-aws-image19.png[DR vmc AWS 影像 19.]
...
+
image::dr-vmc-aws-image20.png[DR vmc AWS 影像 20]
...
image::dr-vmc-aws-image23.png[DR vmc AWS 影像 23.]
...
image::dr-vmc-aws-image24.png[DR vmc AWS 影像 24]
...
image::dr-vmc-aws-image25.png[DR vmc AWS 影像 25]
...
+
image::dr-vmc-aws-image33.png[DR vmc AWS 影像 33.]

. 在「新增備份儲存庫」精靈中、選取「物件儲存設備」、然後選取「Amazon S3」。這會啟動「新增物件儲存庫」精靈。
+
image::dr-vmc-aws-image34.png[DR vmc AWS 影像 34]

. 提供物件儲存庫的名稱、然後按「Next（下一步）」。
. 在下一節中、提供您的認證資料。您需要AWS存取金鑰和秘密金鑰。
+
image::dr-vmc-aws-image35.png[DR vmc AWS 影像 35]
...
+
image::dr-vmc-aws-image40.png[DR vmc AWS 影像 40]

. 展開「驗證」區段、然後按一下「試用」。
+
image::dr-vmc-aws-image41.png[DR vmc AWS 影像 41.]

. 在UserOperationConttext區域中、填入SnapCenter 「資訊」認證和角色、然後按一下「執行」。
+
image::dr-vmc-aws-image42.png[DR vmc AWS 影像 42.]

. 在下方的「回應」本文中、您可以看到權杖。執行備份程序時、請複製權杖文字以進行驗證。
+
image::dr-vmc-aws-image43.png[DR vmc AWS 影像 43.]
...
+
image::dr-vmc-aws-image44.png[DR vmc AWS 影像 44]

. 展開「/4.6/dissterrecovery /server/Backup」區段、然後按一下「Try it out（試用）」。
+
image::dr-vmc-aws-image45.png[DR vmc AWS 影像 45]
...
+
image::dr-vmc-aws-image46.png[DR vmc AWS 影像 46]
...
image::dr-vmc-aws-image47.png[DR vmc AWS 影像 47]
...
+
image::dr-vmc-aws-image48.png[DR vmc AWS 影像 48]

. 貼上您的授權權杖、然後在「SmDRResterRequest」區段中、貼上備份名稱和次要SnapCenter 伺服器上的本機目錄。
+
image::dr-vmc-aws-image49.png[DR vmc AWS 影像 49]

. 選取「執行」按鈕以開始還原程序。
. 從功能區塊瀏覽至「監控」區段、以檢視還原工作的進度。SnapCenter
+
image::dr-vmc-aws-image50.png[DR vmc AWS 影像 50]

+
image::dr-vmc-aws-image51.png[DR vmc AWS 影像 51.]
...
+
image::dr-vmc-aws-image52.png[DR vmc AWS 影像 52]
...
+
image::dr-vmc-aws-image53.png[DR vmc AWS 影像 53.]

. 選取「Amazon S3」作為「物件儲存類型」。
+
image::dr-vmc-aws-image54.png[DR vmc AWS 影像 54]

. 從Amazon Cloud Storage Services清單中、選取Amazon S3。
+
image::dr-vmc-aws-image55.png[DR vmc AWS 影像 55]

. 從下拉式清單中選取預先輸入的認證資料、或新增認證資料以存取雲端儲存資源。按一下「下一步」繼續。
+
image::dr-vmc-aws-image56.png[DR vmc AWS 影像 56]

. 在「時段」頁面上、輸入資料中心、時段、資料夾及任何所需選項。按一下套用。
+
image::dr-vmc-aws-image57.png[DR vmc AWS 影像 57]
...
+
image::dr-vmc-aws-image58.png[DR vmc AWS 影像 58]

. 建立匯入的資料庫記錄之後、請在摘要畫面中選取「Next（下一步）」、然後選取「Finish（完成）」、開始匯入程序。
+
image::dr-vmc-aws-image59.png[DR vmc AWS 影像 59]

. 匯入完成後、您可以將VM還原至VMware Cloud叢集。
+
image::dr-vmc-aws-image60.png[DR vmc AWS 影像 60]
...
+
image::dr-vmc-aws-image61.png[DR vmc AWS 影像 61.]

. 在完整VM還原精靈的第一頁、視需要修改要備份的VM、然後選取「Next（下一步）」。
+
image::dr-vmc-aws-image62.png[DR vmc AWS 影像 62]

. 在「還原模式」頁面上、選取「還原至新位置」或「使用不同的設定」。
+
image::dr-vmc-aws-image63.png[DR vmc AWS 影像 63]
...
+
image::dr-vmc-aws-image65.png[DR vmc AWS 影像 65]

. 在「網路」頁面上、將VM上的原始網路對應到新目標位置的網路。
+
image::dr-vmc-aws-image66.png[DR vmc AWS 影像 66]

+
image::dr-vmc-aws-image67.png[DR vmc AWS 影像 67]
...
+
image::dr-vmc-aws-image68.png[DR vmc AWS 影像 68]

. 在「Discovery（探索）」索引標籤中、按一下「Discover Portal（探索入口網站）」、然後輸入FSX iSCSI目標的IP位址。
+
image::dr-vmc-aws-image69.png[DR vmc AWS 影像 69]

+
image::dr-vmc-aws-image70.png[DR vmc AWS 影像 70]

. 在「Target」（目標）索引標籤上、按一下「Connect」（連線）、選取「Enable Multi-Path（啟用多重路徑）」（若適用於您的組態）、然後按一下「OK（確定）」連線至
+
image::dr-vmc-aws-image71.png[DR vmc AWS 影像 71.]

. 開啟「電腦管理」公用程式、使磁碟上線。請確認它們保留的磁碟機代號與先前所保留的相同。
+
image::dr-vmc-aws-image72.png[DR vmc AWS 影像 72.]
...
+
image::dr-vmc-aws-image73.png[DR vmc AWS 影像 73.]

. 按一下「Add（新增）」、然後瀏覽至包含SQL Server主要資料庫檔案的資料夾、選取該檔案、然後按一下「OK（確定）」。
+
image::dr-vmc-aws-image74.png[DR vmc AWS 影像 74]

. 如果交易記錄位於不同的磁碟機上、請選擇包含交易記錄的資料夾。
. 完成後、按一下「確定」以附加資料庫。
+
image::dr-vmc-aws-image75.png[DR vmc AWS 影像 75]
...
image::dr-vmc-aws-image76.png[DR vmc AWS 影像 76]
...
....
+
image::dr-vmc-aws-image77.png[DR vmc AWS 影像 77]
...
....
+
image::dr-vmc-aws-image78.png[DR vmc AWS 影像 78]

. 更新Amazon FSX Web用戶端中的交會路徑：
+
image::dr-vmc-aws-image79.png[DR vmc AWS 影像 79]

. 新增交會路徑名稱、然後按一下「Update（更新）」。從Oracle伺服器掛載NFS Volume時、請指定此交會路徑。
+
image::dr-vmc-aws-image80.png[DR vmc AWS 影像 80]
...
+
image::dr-vmc-aws-image81.png[DR vmc AWS 影像 81]

. 從動作功能表中、選取Mount Command（掛載命令）以檢視及複製要在Oracle Linux伺服器上使用的掛載命令。
+
image::dr-vmc-aws-image82.png[DR vmc AWS 影像 82]

+
image::dr-vmc-aws-image83.png[DR vmc AWS 影像 83.]
