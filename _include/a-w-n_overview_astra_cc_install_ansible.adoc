...
. 若要使用Ansible教戰手冊來部署Astra Control Center、您必須安裝有Ansible的Ubuntu / RHEL機器。請依照所述程序進行 link:../automation/automation_ubuntu_debian_setup.html["請按這裡"] 適用於Ubuntu和 link:../automation/automation_rhel_centos_setup.html["請按這裡"] 適用於RHEL。
. 複製裝載可執行內容的GitHub儲存庫。
+
[source, cli]
----
