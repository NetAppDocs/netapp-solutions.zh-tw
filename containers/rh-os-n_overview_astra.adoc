...
.自動[可執行]
--
. 若要使用Ansible教戰手冊來部署Astra Control Center、您需要安裝Ansible的Ubuntu / RHEL機器。請依照程序進行 https://docs.netapp.com/us-en/netapp-solutions/automation/automation_ubuntu_debian_setup.html["請按這裡"] 適用於Ubuntu和程序 https://docs.netapp.com/us-en/netapp-solutions/automation/automation_rhel_centos_setup.html["請按這裡"] 適用於RHEL。
. 複製裝載可執行內容的GitHub儲存庫。
+
[source, cli]
----
