...
. 若要使用部署Astra Control Center的Ansible教戰手冊、您必須安裝有Ansible的Ubuntu / RHEL機器。請遵循此步驟 https://docs.netapp.com/us-en/netapp-solutions/automation/automation_ubuntu_debian_setup.html["程序"^] 適用於Ubuntu及此 https://docs.netapp.com/us-en/netapp-solutions/automation/automation_rhel_centos_setup.html["程序"^] 適用於RHEL。
. 複製裝載可執行內容的GitHub儲存庫。
+
[source, cli]
----
...
link:vtwn_astra_register.html["下一步：註冊您的Tanzu Kubernetes叢集。"]
