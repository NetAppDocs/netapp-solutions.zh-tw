...
vSphere搭配Tanzu（vSphere Pod）

* vSphere原生Pod採用指定的虛擬硬體、可在以照片為基礎的精簡層執行、以實現完整的隔離。
* 需要NSxT-T、但這可額外支援諸如港映像登錄等功能。
* 使用虛擬的監督者叢集（例如TKGS）、直接在ESXi節點上執行Pod、在vSphere 7.0U1中部署及管理。
* vSphere管理員完全整合vSphere、提供最高可見度與控制能力。
* 隔離式CRX型Pod、提供最高層級的安全性。
* 僅支援vSphere SCSI持續儲存/不支援第三方儲存協調器。
