...
如需VMware vSphere的詳細資訊、請參閱 https://["VMware vSphere網站"^]。
...
Google Cloud會透過其Anthos Ready平台合作夥伴方案、定期要求更新新版Anthos的合作夥伴伺服器平台驗證。您可以找到目前已驗證的伺服器平台清單、以及所支援的Anthos版本 https://["請按這裡"^]。
...
image:a-w-n_Anthos-1.14-vsphere7_validation.png["錯誤：缺少圖形影像"]
...
https://["vSphere 6.7說明文件：使用DRS關聯性規則"^]。https://["vSphere 7.0文件：使用DRS關聯性規則"^]。


NOTE: Anthos在每個個別的「叢集.yaml」檔案中都有一個組態選項、可根據您環境中的ESXi主機數量、自動建立可啟用或停用的節點關聯規則。

link:a-w-n_anthos_BM.html["下一步：裸機上的安索斯。"]
