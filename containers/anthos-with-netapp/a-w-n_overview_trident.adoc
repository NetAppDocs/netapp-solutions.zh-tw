...
NOTE: 依照預設、不會在GKE管理工作站上安裝Helm。可從下載、以二進位格式與Ubuntu搭配使用 https://helm.sh/docs/intro/install/["Helm安裝頁面"]。
...
link:a-w-n_trident_ontap_nfs.html["下一步：NetApp ONTAP 不再是NFS。"]
