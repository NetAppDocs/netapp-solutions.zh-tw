...
NOTE: Trident Protect 需要登錄來裝載 Astra 容器所需的映像。下節說明在 Red Hat OpenShift 叢集上設定私有登錄，並推送支援安裝 Trident Protect 所需的映像的步驟。
...
----
. 如果您使用入口操作員OpenShift登錄路由的預設TLS憑證、則可以使用下列命令擷取TLS憑證。
+
[listing]
----
