...
link:xcp-bp-migration-workflow-overview.html["上一個：移轉工作流程。"]
...
NOTE: 如需XCP檔案分析的高層架構、統計資料檢視等GUI型儀表板檢視、以及檔案發佈檢視詳細資料的詳細資訊、請參閱部落格文章 https://["NetApp XCP 1.6提供開放式檔案分析與基礎架構改善"^]。
...
link:xcp-bp-deployment-steps.html["下一步：部署步驟。"]
