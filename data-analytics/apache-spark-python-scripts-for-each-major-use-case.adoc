...
link:apache-spark-hybrid-cloud-solution.html["先前版本：混合雲解決方案。"]

下列三個Python指令碼分別對應三個測試的主要使用案例。第一是"entitment_section_sparknlp.py"。
...
....
第二個指令碼是「keras」（keras）、「SPARK」（horovod_rossmann_imer.py）。
...
if __name__ == '__main__':
    ...
    spark.stop()
....
第三個指令碼是「rrun _crite_criteo_site.py」。
...
....
link:apache-spark-conclusion.html["下一步：結論。"]
