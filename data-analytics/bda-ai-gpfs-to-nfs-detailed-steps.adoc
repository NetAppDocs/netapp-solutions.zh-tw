...
summary: 本節提供使用NetApp XCP設定GPFS並將資料移至NFS所需的詳細步驟。 
---
= GPFS至NFS的詳細步驟
:hardbreaks:
:allow-uri-read: 
...
link:bda-ai-business-benefits.html["上一篇：商業效益。"]
...
. 在其中一部伺服器上下載並安裝適用於Linux的Spectrum Scale Data Access。
+
....
...
....
. 指定GPFS要使用的遠端Shell二進位檔；使用「-r argument」。
+
....
...
[root@mastr-51 installer]#
....
. 指定要在所有GPFS節點上設定的連接埠範圍；使用「-e argument」。
+
....
...
link:bda-ai-mapr-fs-to-ontap-nfs.html["下一步：從MapR-FS移轉ONTAP 到Sfnfs。"]
