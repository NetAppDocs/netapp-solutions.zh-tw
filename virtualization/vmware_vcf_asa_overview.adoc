...
如需更多關於使用 BlueXP 的 3-2-1 備份策略的資訊、請造訪 https://docs.netapp.com/us-en/netapp-solutions/ehc/bxp-scv-hybrid-solution.html["適用於 VMware 的 3-2-1 Data Protection 、搭配 SnapCenter 外掛程式、以及適用於 VM 的 BlueXP 備份與還原"]。
...
* * 使用 ONTAP 工具在 VCF 管理網域 * 中部署 iSCSI 資料存放區。按一下 https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_mgmt_iscsi.html["* 此處 *"] 部署步驟。
* * 使用 ONTAP 工具在 VI 工作負載網域 * 中部署 vVols （ iSCSI ）資料存放區。按一下 https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_wkld_vvols.html["* 此處 *"] 部署步驟。
* * 設定 NVMe over TCP 資料存放區、以用於 VI 工作負載網域 * 。按一下 https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_supp_wkld_nvme.html["* 此處 *"] 部署步驟。
* * 部署並使用適用於 VMware vSphere 的 SnapCenter 外掛程式來保護及還原 VI 工作負載網域中的虛擬機器 * 。按一下 https://docs.netapp.com/us-en/netapp-solutions/virtualization/vmware_vcf_asa_scv_wkld.html["* 此處 *"] 部署步驟。
