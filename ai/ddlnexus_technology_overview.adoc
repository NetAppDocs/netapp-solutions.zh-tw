...
Trident 可在所有熱門的 NetApp 儲存平台，公有雲或內部部署中，使用和管理儲存資源，包括 ONTAP （ AFF ， FAS ， Select ， Cloud ， Amazon FSX ONTAP ）， Element 軟體（ NetApp HCI ， SolidFire ）， Azure NetApp Files 服務，以及 Google Cloud NetApp Volumes 。Trident 是符合 Container Storage Interface （ CSI ）規範的動態儲存協調器、可與 Kubernetes 原生整合。
