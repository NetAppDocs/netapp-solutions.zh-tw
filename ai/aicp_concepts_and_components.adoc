...
link:https://docs.netapp.com/us-en/trident/index.html["Astra Trident"^] 可在所有常見的 NetApp 儲存平台、公有雲或內部部署（包括 ONTAP ）（ AFF 、 FAS 、 Select 、 Cloud 、 Amazon FSX for NetApp ONTAP ）、 Azure NetApp Files 服務和 Google Cloud NetApp Volumes 。Astra Trident 是符合 Container Storage Interface （ CSI ）規範的動態儲存協調器、可與 Kubernetes 原生整合。
