...
+
** 您可以找到安裝與組態指示 https://["請按這裡"^]。


* 適用於Kubernetes的NetApp DataOps工具套件
+
** 您可以找到安裝指示 https://["請按這裡"^]。
...
+
有多種方法可將資料載入至PVc。如果您的資料目前儲存在S3相容的物件儲存平台、例如NetApp StorageGRID 功能區或Amazon S3、您就可以使用 https://["NetApp DataOps Toolkit S3 Data Mover功能"^]。另一種簡單的方法是建立JupyterLab工作區、然後透過JupyterLab網頁介面上傳檔案、如「」一節中步驟3至5所述<<案例1–JupyterLab的隨需推斷>>。」
...
....
. 將您的模型儲存在中的新持續磁碟區上 https://["格式"^] NVIDIA Triton Inference伺服器也能辨識這點。
+
